...
const shot = async (name) => {
  await page.screenshot({ path: `/workspace/screenshots/v43-${name}.png`, fullPage: false });
};
...
try {
  ...
  await shot("tabor");
  const tabor = await page.evaluate(() => {
    const hero = document.querySelector(".kauba-hero");
    const prikaz = document.querySelector(".hub-prikaz");
    const sadrzaj = document.querySelector(".kauba-tabor-sadrzaj");
    const hr = hero?.getBoundingClientRect();
    const pr = prikaz?.getBoundingClientRect();
    const sr = sadrzaj?.getBoundingClientRect();
    return {
      taborGrad: document.querySelectorAll(".tabor-grad").length,
      hubPrikaz: document.querySelectorAll(".hub-prikaz").length,
      labav: !!document.querySelector(".kauba-hero-labav"),
      meni: !!document.querySelector(".kauba-meni-stick"),
      heroH: hr ? Math.round(hr.height) : 0,
      prikazH: pr ? Math.round(pr.height) : 0,
      sadrzajH: sr ? Math.round(sr.height) : 0,
      sadrzajY: sr ? Math.round(sr.top) : 0,
    };
  });

  await page.getByRole("button", { name: /^Zadaci$/i }).click();
  await page.waitForTimeout(300);
  await dismiss();
  await page.getByText("Karavana").first().waitFor({ state: "visible" });
  const kola = await page.locator(".karavana-kola-slika").count();
  const staza = await page.locator(".karavana-staza").count();
  await shot("karavana");
